...
# --- IAM User Credential
# Only have Read & Put Permission on specific bucket.
IAM_ID = 'AKIAYRBXPIB6WWA7OT7F'
IAM_PW = 'rnWTQU7s4LSS5v4Id4A+PrRXE1e8H0a3G+aoGEYF'
...
def Make_output_Info(fin_cell):
    ...
    data = {}
    # 기본정보 에러
    baseErrCnt = fin_cell[fin_cell.iloc[:, 12].name].notnull().sum()
    data["baseErrCnt"] = str(baseErrCnt)
    # 원료비중 오류
    baseMatErrCnt1 = fin_cell[fin_cell.iloc[:, 14].name].notnull().sum()
    baseMatErrCnt2 = fin_cell[fin_cell.iloc[:, 15].name].notnull().sum()
    baseMatErrCnt3 = fin_cell[fin_cell.iloc[:, 16].name].notnull().sum()
    baseMatErrCntSum = sum([baseMatErrCnt1, baseMatErrCnt2, baseMatErrCnt3])
    
    print("원료비중 오류 {0} \n".format("문제 있음" if baseMatErrCntSum > 0 else "문제 없음"))
    data["baseMatErr"] = "문제 있음" if baseMatErrCntSum > 0 else "문제 없음"

    # 위생허가 구제 오류
    hygieneErrCnt1 = fin_cell[fin_cell.iloc[:, 17].name].notnull().sum()
    hygieneErrCnt2 = fin_cell[fin_cell.iloc[:, 18].name].notnull().sum()
    hygieneErrCnt3 = fin_cell[fin_cell.iloc[:, 19].name].notnull().sum()
    hygieneErrCnt4 = fin_cell[fin_cell.iloc[:, 20].name].notnull().sum()
    hygieneErrCnt5 = fin_cell[fin_cell.iloc[:, 21].name].notnull().sum()
    hygieneErrCnt6 = fin_cell[fin_cell.iloc[:, 22].name].notnull().sum()
    hygieneErrCnt7 = fin_cell[fin_cell.iloc[:, 23].name].notnull().sum()
    hygieneErrCnt8 = fin_cell[fin_cell.iloc[:, 24].name].notnull().sum()
    # hygieneErrCnt9 = fin_cell[fin_cell.iloc[:, 25].name].notnull().sum()
    hygieneErrCnt9 = 0
   
    # print(fin_cell.iloc[:, 25].name)
    hygieneErrCntSum = sum([hygieneErrCnt1, hygieneErrCnt2, hygieneErrCnt3, hygieneErrCnt4,
                           hygieneErrCnt5, hygieneErrCnt6, hygieneErrCnt7, hygieneErrCnt8, hygieneErrCnt9])
    print("위생허가 구제 오류 {0} \n".format("문제 있음" if hygieneErrCntSum > 0 else "문제 없음"))

    # 통관 위험
    customRisk = fin_cell.loc[(fin_cell[fin_cell.iloc[:, 26].name].notnull()) & (fin_cell[fin_cell.iloc[:, 28].name].notnull())]
    print("통관 위험 에러 {0}건 \n".format(len(customRisk)))

    # 마케팅리스크
    marketingRisk = fin_cell.loc[(fin_cell[fin_cell.iloc[:, 29].name].str.startswith('3')) | (fin_cell[fin_cell.iloc[:, 30].name].str.contains('20가지 주의성분')) | (fin_cell[fin_cell.iloc[:, 31].name].notnull())]
    # marketingRisk.to_excel("marketingRisk.xlsx", index=False)
    print("마케팅리스크 {0}건\n".format(len(marketingRisk)))
...
# Full_code_fin("C:/cnode/1. derma/위생허가/복합성분표 테스트 자료.xlsx")
